...
/// A a combination of a judgement and an expression, e.g. "x0 -> x0 is provable"
///
/// The _judgement_ is given in form of an integer, but often represents some meaning, like "this
/// expression is provable" or "this expression is syntactically correct".
///
/// The _expression_ is a sequence of integers, with the following meaning:
/// * A non-negative integer stands for a _variable_ uniquely identified by this integer
/// * A integer less or equal to `-2` stands for an _operator_. The operator has no associated
...
/// * The integer `-1` is a _special constant_ node to be used for operators with arity 0 or 1
///
/// A valid expression must have of one of the following formats:
/// * A _variable_, or the _special constant_ `-1`
/// * A _operator_, followed by two valid expression (the operands)
///
/// For example a valid sequences would be `[-2, 0, -2, 1, 0]` or `[-3, -2, 0, 0, -1]` which could
/// stand for "x0 -> (x1 -> x0)" or "not (x0 -> x0)" respectively.
///
/// To create operators of _arity 0 and 1_, one can use the _special constant_ `-1`. For example
/// `-2, 0, -1` is a valid expression which could stand for "not x0".
///
/// To create operators of _arity 3 or higher_, one can split the up into multiple operators of artiy
/// 2.
/// For example `-2, 0, -3, 1, 2` is a valid sequence which could stand for "if x0 then x1 else x2"
pub trait Statement {
